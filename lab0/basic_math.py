...
def functions(a_1, a_2):
    ...
    if A == 0:
        if B == 0:
            return [] if C != 0 else None  # Нет решений или бесконечно много
        else:
            x = -C / B  # Одно решение
...
def skew(x):
    ...
    if std_dev == 0:
        return float('nan')  # Невозможно рассчитать асимметрию
...
def kurtosis(x):
    ...
    if std_dev == 0:
        return float('nan')  # Невозможно рассчитать эксцесс
