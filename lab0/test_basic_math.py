...
class MATHTestCase(unittest.TestCase):
    def test_matrix_multiplication(self):
        ...
        b1 = [[7, 8], [9, 10], [11, 12]]
        self.assertEqual(
            basic_math.matrix_multiplication(a1, b1), [[58, 64], [139, 154]]
        )
    ...
    def test_skew(self):
        ...
        self.assertEqual(
            basic_math.skew(random_floats), round(scipy.stats.skew(random_floats), 2)
        )
        self.assertEqual(
            basic_math.skew(random_integers),
            round(scipy.stats.skew(random_integers), 2),
        )
    ...
    def test_kurtosis(self):
        ...
        random_integers = [random.randint(1, 99) for _ in range(10000)]
        self.assertEqual(
            basic_math.kurtosis(random_floats),
            round(scipy.stats.kurtosis(random_floats), 2),
        )
        self.assertEqual(
            basic_math.kurtosis(random_integers),
            round(scipy.stats.kurtosis(random_integers), 2),
        )
